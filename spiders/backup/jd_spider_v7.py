...
def run_js_file(js_file):
    ...
    result = subprocess.run(
        ['osascript', '-e', f'tell application "Safari" to do JavaScript "{js_code}" in current tab of front window'],
...
def get_detail_price(url):
    ...
    js = '''
    var price = 0;
    var priceElem = document.querySelector('.jd-price .price') || document.querySelector('.p-price i');
    if(priceElem) {
        var text = priceElem.innerText;
        var match = text.match(/(\\d+\\.?\\d*)/);
        if(match) price = parseFloat(match[1]);
    }
    if(price === 0) {
        var bodyText = document.body.innerText;
        var match = bodyText.match(/[¥￥](\\d+\\.?\\d*)/);
        if(match) price = parseFloat(match[1]);
    }
    price;
    '''

...
def run_js_file():
    """执行临时文件中的 JavaScript"""
    cmd = f'''osascript -e 'tell application "Safari" to set js to do shell script "cat {JS_FILE}"' -e 'do JavaScript js in current tab of front window' '''
...
def main():
    ...
    # 登录检查
    with open(JS_FILE, 'w') as f:
        f.write("document.cookie.indexOf('pin=') >= 0")
    is_login = 'true' in run_js_file()

...
def run_js(js_code):
    """执行 JavaScript"""
    script = f'tell application "Safari" to do JavaScript "{js_code}" in current tab of front window'
...
if(m) {
    ...
    for(var i=0; i<items.length; i++) {
        ...
        if(idMatch) {
            products.push({id: idMatch[1], title: title, url: url});
        }
    }
}
    ...
    for p in products:
        ...
        if not cursor.fetchone():
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (p['id'], p['url'], p['title'][:500], shop_name, shop_url,
